...
local function hl(group, fg, bg, attr)
    ...
    -- Handle Colors
    if fg then opts.fg = fg.gui; opts.ctermfg = fg.cterm end
    if bg then opts.bg = bg.gui; opts.ctermbg = bg.cterm end
...
hl('StatusLine',   c.black,        c.white)
hl('StatusLineNC', c.white,        c.dark_gray)
...
-- 6. Diagnostics (LSP)
hl('DiagnosticError', c.bright_red,   nil)
